...
DO $$ BEGIN
  ...
  IF NOT EXISTS (
    ...
    -- Only create table if brands table exists
    ...
    ELSE
      -- Create table without foreign key constraint if brands table doesn't exist
      CREATE TABLE user_brand_permissions (
        user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
DO $$ BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND policyname = 'Users can view own brand assignments'
  ) THEN
    CREATE POLICY "Users can view own brand assignments" 
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_brand_permissions' 
    AND policyname = 'Admins can manage brand assignments'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_brand_permissions' 
    AND policyname = 'Brand admins can manage their brand assignments'
  ) THEN
    CREATE POLICY "Brand admins can manage their brand assignments" 
    ON user_brand_permissions
    FOR ALL USING (
      EXISTS (
