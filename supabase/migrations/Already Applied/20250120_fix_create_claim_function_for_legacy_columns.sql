-- Fix the create_claim_with_associations function to handle legacy columns
-- This is needed because the claims table still has a check constraint requiring these columns
...
COMMENT ON FUNCTION "public"."create_claim_with_associations" IS 'Creates a claim with proper associations to products, ingredients, and countries using junction tables. Maintains backward compatibility with legacy single-value columns.';