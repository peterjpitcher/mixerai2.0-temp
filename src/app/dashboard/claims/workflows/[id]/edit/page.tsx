  ...
  const fetchWorkflow = async () => {
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        setStatus(workflow.is_active ? 'active' : 'inactive');
        
        // Transform steps to match our state structure
        const transformedSteps: WorkflowStep[] = workflow.steps.map(step => ({
          id: step.id,
          name: step.name,
          description: step.description || '',
          role: step.role,
          assignees: step.assigned_users || [],
          step_order: step.step_order
        }));
        
        setSteps(transformedSteps);
        setAssigneeInputs(new Array(transformedSteps.length).fill(''));
      } else {
    ...
    try {
      const payload = {
        ...
        steps: steps.map((step, index) => ({
          ...
          role: step.role,
          step_order: index + 1,
          assigned_user_ids: step.assignees.map(a => a.id.startsWith('email_') ? a.email : a.id),
        })),
      };
      ...
      if (data.success) {
        ...
        router.push(`/dashboard/claims/workflows/${workflowId}`);
      } else {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Card>
          <CardFooter className="flex justify-end space-x-3">
            <Button type="button" variant="outline" onClick={() => router.push(`/dashboard/claims/workflows/${workflowId}`)} disabled={isSaving}>
              Cancel
            </Button>
