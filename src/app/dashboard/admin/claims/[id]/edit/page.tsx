...
export default function EditClaimPage() {
    ...
    async function fetchClaimDetails() {
      try {
        ...
        if (result.success && result.data) {
          ...
          if (claim.country_code !== GLOBAL_COUNTRY_CODE) {
  ...
  const isMarketSpecificClaimUsedAsReplacement = formData.country_code !== GLOBAL_COUNTRY_CODE && linkedOverrides.length > 0;
  ...
  if (!id && !isLoading) {
    return (
      <div className="px-4 sm:px-6 lg:px-8 py-6 flex flex-col items-center justify-center min-h-[calc(100vh-200px)]">
        ...
        <Button asChild variant="outline"><Link href="/dashboard/admin/claims"><ArrowLeft className="mr-2 h-4 w-4" />Back</Link></Button>
      </div>
    );
  }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Select name="country_code" value={formData.country_code} onValueChange={(value) => handleSelectChange('country_code' as keyof ClaimEditFormData, value)}
                    disabled={isMarketSpecificClaimUsedAsReplacement}
                >
                    <SelectTrigger className={formErrors.country_code ? "border-red-500" : ""}><SelectValue placeholder="Select country" /></SelectTrigger>
                    <SelectContent className="max-h-60">
                        <SelectItem value={GLOBAL_COUNTRY_CODE}>{GLOBAL_COUNTRY_NAME}</SelectItem>
