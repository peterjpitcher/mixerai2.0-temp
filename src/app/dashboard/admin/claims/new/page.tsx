...
import { COUNTRY_CODES } from "@/lib/constants/country-codes"; // Assuming this file exists
...
interface GlobalClaimBrand { id: string; name: string; }
interface Product { id: string; name: string; global_brand_id?: string; global_brand_name?: string; }
...
export default function NewClaimPage() {
  ...
  useEffect(() => {
    async function fetchEntitiesForSelect() {
      ...
      try {
        const [brandsRes, productsRes, ingredientsRes] = await Promise.allSettled([
          fetch('/api/global-claim-brands').then(res => res.json()),
          fetch('/api/products').then(res => res.json()),
          fetch('/api/ingredients').then(res => res.json()),
        ]);

        if (brandsRes.status === 'fulfilled' && brandsRes.value.success) setGlobalBrands(brandsRes.value.data || []);
        else console.error("Failed to load global brands:", brandsRes.status === 'rejected' ? brandsRes.reason : brandsRes.value.error);
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.level === 'brand' && !formData.global_brand_id) newErrors.global_brand_id = "Global Brand is required for brand-level claims.";
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (formData.level === 'brand') payload.global_brand_id = formData.global_brand_id;
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Select name="country_code" value={formData.country_code} onValueChange={(value) => handleSelectChange('country_code', value)}>
                    <SelectTrigger className={errors.country_code ? "border-red-500" : ""}><SelectValue placeholder="Select country" /></SelectTrigger>
            ...
            {formData.level === 'brand' && (
              <div className="space-y-2">
                ...
                <Select name="global_brand_id" value={formData.global_brand_id || ""} onValueChange={(value) => handleSelectChange('global_brand_id', value)} disabled={isLoadingBrands}>
                  <SelectTrigger className={errors.global_brand_id ? "border-red-500" : ""}><SelectValue placeholder={isLoadingBrands ? "Loading brands..." : "Select Global Brand"} /></SelectTrigger>
                  <SelectContent>
                    {globalBrands.length === 0 && !isLoadingBrands ? <SelectItem value="" disabled>No brands available</SelectItem> : 
                     globalBrands.map(brand => <SelectItem key={brand.id} value={brand.id}>{brand.name}</SelectItem>)}
                  </SelectContent>
                </Select>
                ...
                <Select name="product_id" value={formData.product_id || ""} onValueChange={(value) => handleSelectChange('product_id', value)} disabled={isLoadingProducts}>
                  ...
                  <SelectContent>
                    {products.length === 0 && !isLoadingProducts ? <SelectItem value="" disabled>No products available</SelectItem> : 
                     products.map(product => <SelectItem key={product.id} value={product.id}>{product.name} {product.global_brand_name ? `(${product.global_brand_name})` : ''}</SelectItem>)}
                  </SelectContent>
                </Select>
