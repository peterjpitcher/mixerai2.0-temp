  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const fetchedBrands: GlobalClaimBrand[] = brandsData.success ? brandsData.data : [];
        ...
        const enrichedClaims = (claimsData.data as Claim[]).map(claim => {
          ...
          if (claim.level === 'brand' && claim.global_brand_id) {
            entity_name = fetchedBrands.find(b => b.id === claim.global_brand_id)?.name || 'Unknown Brand';
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      <PageHeader 
        ...
        actions={
          <Button asChild>
            <Link href="/dashboard/admin/claims/new">
            ...
            <Card key={claim.id} className="flex flex-col">
              ...
              <CardFooter className={`flex justify-end space-x-2 ${!claim.description ? 'pt-4' : ''}`}>
                <Button variant="outline" size="sm" asChild>
                  <Link href={`/dashboard/admin/claims/${claim.id}/edit`}>
