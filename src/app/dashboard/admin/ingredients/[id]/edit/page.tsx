    ...
    async function fetchIngredientDetails() {
      try {
        ...
        if (result.success && result.data) {
          const ingredient: Ingredient = result.data;
          setFormData({
          ...
          throw new Error(result.error || "Failed to parse ingredient details.");
        }
      } catch (err) {
        const errorMessage = (err as Error).message || "An unexpected error occurred.";
        console.error("Error fetching ingredient details:", errorMessage);
        setError(errorMessage);
        toast.error("Failed to load ingredient details.", { description: errorMessage }); 
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        router.push("/dashboard/admin/ingredients");
      } else {
    ...
    return (
      <div className="px-4 sm:px-6 lg:px-8 py-6 flex flex-col items-center justify-center min-h-[calc(100vh-200px)]">
        ...
        <Button asChild variant="outline">
          <Link href="/dashboard/admin/ingredients">
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <PageHeader 
        ...
        description={`Editing ingredient: ${formData.name || id}`}
      />
