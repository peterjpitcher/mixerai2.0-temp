...
export async function middleware(request: NextRequest) {
  ...
  const rateLimitType = getRateLimitType(pathForRateLimit);
  ...
  if (supabaseUrl && supabaseAnonKey) {
    ...
    if (refreshedUser?.id) {
      if (sessionId) {
        // Validate existing session using Redis
        ...
        if (!isSessionValid) {
          ...
          const renewed = await renewSessionRedis(sessionId);
          if (!renewed) {
            // If renewal fails, create a new session
            const newSession = await createSessionRedis(refreshedUser as User, {
              userAgent: request.headers.get('user-agent') || undefined,
              ipAddress: request.headers.get('x-forwarded-for') || request.headers.get('x-real-ip') || undefined,
            });
