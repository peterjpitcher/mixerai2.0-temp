...
CREATE OR REPLACE FUNCTION "public"."get_user_templates"("p_user_id" "uuid", "p_brand_id" "uuid" DEFAULT NULL::"uuid") RETURNS TABLE("id" "uuid", "name" "text", "description" "text", "category" "text", "template_structure" "jsonb", "content_type_id" "uuid", "brand_id" "uuid", "created_by" "uuid", "created_at" timestamp with time zone, "updated_at" timestamp with time zone, "is_active" boolean, "usage_count" integer, "brand_name" "text", "content_type_name" "text", "creator_name" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
    v_user_role text;
...
ALTER FUNCTION "public"."get_user_templates"("p_user_id" "uuid", "p_brand_id" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."get_user_templates"("p_user_id" "uuid", "p_brand_id" "uuid") IS 'Returns templates accessible to a user based on their global role (admin) or brand permissions';
...
COMMENT ON FUNCTION "public"."update_content_workflow_status"("p_content_id" "uuid", "p_user_id" "uuid", "p_action" "text", "p_comments" "text", "p_new_assignee_id" "uuid", "p_version_data" "jsonb") IS 'Updates content workflow status and creates audit trail. Returns success status, error message, new status, and new step order.';
    ...
    if p_brand_permissions is not null then
        ...
        if jsonb_array_length(p_brand_permissions) > 0 then
            ...
            select
                p_user_id,
                (perm->>'brand_id')::uuid,
                (perm->>'role')::public.user_brand_role_enum  -- Cast to enum type
...
CREATE POLICY "Brand admins can manage brand master claim brand links" ON "public"."brand_master_claim_brands" USING ((("auth"."uid"() IN ( SELECT "user_brand_permissions"."user_id"
   FROM "public"."user_brand_permissions"
  WHERE (("user_brand_permissions"."brand_id" = "brand_master_claim_brands"."brand_id") AND ("user_brand_permissions"."role" = 'admin'::"public"."user_brand_role_enum")))) OR (EXISTS ( SELECT 1
   FROM "auth"."users"
...
CREATE POLICY "Users can create claim ingredients" ON "public"."claim_ingredients" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."claims" "c"
  WHERE ("c"."id" = "claim_ingredients"."claim_id"))));
...
CREATE POLICY "Users can delete claim ingredients" ON "public"."claim_ingredients" FOR DELETE USING ((EXISTS ( SELECT 1
   FROM "public"."claims" "c"
  WHERE ("c"."id" = "claim_ingredients"."claim_id"))));
...
CREATE POLICY "Users can view brand master claim brand links" ON "public"."brand_master_claim_brands" FOR SELECT USING ((("auth"."uid"() IN ( SELECT "user_brand_permissions"."user_id"
   FROM "public"."user_brand_permissions"
  WHERE ("user_brand_permissions"."brand_id" = "brand_master_claim_brands"."brand_id"))) OR (EXISTS ( SELECT 1
   FROM "auth"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND (("users"."raw_user_meta_data" ->> 'role'::"text") = 'admin'::"text"))))));
...
CREATE POLICY "Users can view claim ingredients" ON "public"."claim_ingredients" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."claims" "c"
  WHERE ("c"."id" = "claim_ingredients"."claim_id"))));
...
GRANT ALL ON FUNCTION "public"."get_user_templates"("p_user_id" "uuid", "p_brand_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_user_templates"("p_user_id" "uuid", "p_brand_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_user_templates"("p_user_id" "uuid", "p_brand_id" "uuid") TO "service_role";
